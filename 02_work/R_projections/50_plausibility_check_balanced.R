...
check_extreme_trend_changes <- function(forecast_names,
  ...
  past_trends <- past_population %>%
    ...
    mutate(max_change = quantile(past_trend, (1-prob)),
           min_change = quantile(past_trend, prob)) %>%
  ...
  for (i in seq_along(list_of_forecasts)) {
    ...
    extreme_count <- c(
      ...
      future_trends %>% 
        filter(population_size_group == "< 500", 
               future_trend <  0, 
               future_trend <  min_change1 * prob) %>% 
        nrow(),
      future_trends %>% 
        filter(population_size_group == "500-1000", 
               future_trend <  0, 
               future_trend <  min_change2 * prob) %>% 
        nrow(),
      future_trends %>% 
        filter(population_size_group == "1000-2000", 
               future_trend <  0, 
               future_trend <  min_change3 * prob) %>% 
        nrow(),
      future_trends %>% 
        filter(population_size_group == "2000-5000", 
               future_trend <  0, 
               future_trend <  min_change4 * prob) %>% 
        nrow(),
      future_trends %>% 
        filter(population_size_group == "5000-20000", 
               future_trend <  0, 
               future_trend <  min_change6 * prob) %>% 
        nrow(),
      future_trends %>% 
        filter(population_size_group == "20000-50000", 
               future_trend <  0, 
               future_trend <  min_change7 * prob) %>% 
        nrow(),
      future_trends %>% 
        filter(population_size_group == "> 50000", 
               future_trend <  0, 
               future_trend <  min_change5 * prob) %>% 
        nrow()
    )
...
trend_change_df %>%
  ...
  write.csv2(file = file.path(wd_res, "plausibility_check_trends_2025-2035_balanced_smaller_prob.csv"))
